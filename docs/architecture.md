...
Task 3 adds fail-closed Redis-compatible controls without any process-local fallback. Nonce claim uses one `SET NX EX`; QPS uses a single-key Lua sliding window with Redis `TIME`; concurrency acquire, heartbeat, aggregate observation and exact-owner release use atomic sorted-set scripts scored by server-side lease expiry. Observation prunes stale leases and returns only occupied count plus earliest/latest expiry, never owner members. Caller, credential, nonce, request and operation identifiers are SHA-256 encoded before they enter Redis keys or members.
...
Admission order is fixed: raw request bounds, HMAC authentication, nonce claim, endpoint/model policy, QPS, concurrency lease, SQL request/Token reservation, Open WebUI credit reservation, Bifrost inference, credit settlement, SQL terminal event, then exact-owner concurrency release. An absolute-schedule lease guard covers non-stream and pre-stream awaits; lost ownership cancels the in-flight await and fails closed before recovered Redis can admit overlapping work. Credential/client state is re-read under the SQL reservation transaction so a post-signature revoke or expiry cannot race through quota admission.
...
For streaming, the private Bifrost payload forces `stream_options.include_usage=true`. An incremental decoder validates complete SSE frames and requires one total-matching usage snapshot followed by `[DONE]`. Streaming continues the same absolute lease schedule used during admission even when chunks are continuous, and each stream heartbeat also refreshes the pending operation timestamp. Credit settlement and the SQL terminal event commit before `[DONE]` is forwarded. A disconnect or provider/protocol failure settles only known usage; otherwise it cancels without charge. Errors after HTTP headers use a sanitized SSE error frame, while admission failures remain JSON errors.

Every public response uses the bounded contract error envelope and a server-generated `X-Zangpu-Request-Id`; admitted chat responses also carry QPS and concurrency limit/remaining/reset headers. A concurrency rejection returns stable 429 with zero remaining and bounded `Retry-After`. Events contain stable codes, measured milliseconds, actual Token/charge counters, stream mode and `quota_overrun`, never prompts, answers, Secrets, signatures, nonces, virtual keys or raw upstream errors. `ExternalChatRecoveryWorker.run_once()` is a bounded deployment-scheduled reconciliation pass: it uses Open WebUI operation status plus exact local usage evidence to settle or cancel stale pending operations, never calls Bifrost and never estimates usage. Independent SDK/cURL examples and aggregate-only k6 load tooling exercise these public contracts without importing backend internals.
