...
The administrator site now provides deployment-token login, caller inventory and creation, one-time Secret delivery, redacted credential rotation/revocation, caller disable, binding-sync visibility, endpoint/model permissions, request/Token quota editing, live caller concurrency, model-pool/queue capacity observability, and redacted event/trend/CSV workflows. Browser state keeps only the session-issued CSRF value in memory; it never stores the administrator login token, caller Secret, Bifrost key material or Open WebUI balance.
